...
function report(command, options, callback) {
  execute(command, options.params, function(err, result) {
    if (options.reporter){
      var reporter = require('./reporter/' + options.reporter);
      reporter.report(err, result);
    }
    callback(err, result);
  });
...
exports.gjslint = function(options, callback) {
 report('gjslint', options, callback);
};
