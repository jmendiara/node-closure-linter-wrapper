...
var exec = require('child_process').exec,
    ...
    checkedPythonPath = Object.create(null);
...
var checkPython = function checkPython(pythonPath, callback) {
  if (checkedPythonPath[pythonPath]) {
    callback(null);

    return;
  }

  exec(pythonPath + ' --version',
    function(err, stdout, stderr) {
      if (err !== null) {
        ...
        checkedPythonPath[pythonPath] = true;
...
var parseResult = function parseResult(text, callback) {
  var errors = [], i = 0, l, errBlock, match, match2, lines;
...
function execute(program, options, callback) {
  ...
  checkPython(pythonPath, function(err) {
    if (err) {
      callback(err);
      return;
    }
    var flags = options.flags || [],
        src = options.src || [],
        params = [],
        cmd = path.normalize(pythonPath + ' "' + __dirname +
            '/../tools/' + program + '.py"'),
        fullCommand;
