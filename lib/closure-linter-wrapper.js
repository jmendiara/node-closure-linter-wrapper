...
var parseResult = function parseResult(text, callback) {
  var errors = [], i = 0, l, errBlock, match, match2, lines = text.split('\n');
  ...
  execute('gjslint', options, function(err, result) {
    if (options.reporter && options.reporter.name) {
      var reporter = require('./reporter/' + options.reporter.name),
        report;
      report = reporter.reportGJSLint(err, result, options.reporter);
      if (options.reporter.dest) {
        fs.writeFileSync(options.reporter.dest, report, 'utf-8', function(err) {
          if (err) {
            console.log(err);
          } else {
            console.log('File written in ' + options.reporter.dest);
          }
        });
      }
    }
